	...
	public class pessoaDAO extends DAO {
		...
		public boolean insert(Pessoa pessoa) {
			...
			try {  
				...
				String sql = "INSERT INTO public.usuario (codigo, login, senha, sexo) "
					       + "VALUES ("+pessoa.getCodigo()+ ", '" + pessoa.getLogin() + "', '"  
					       + pessoa.getSenha() + "', '" + pessoa.getSexo() + "');";
		...
		public Pessoa get(int codigo) {
			...
			try {
				...
				String sql = "SELECT * FROM public.usuario WHERE id=" + codigo;
				System.out.println(sql);
		...
		private List<Pessoa> get(String orderBy) {	
			...
			try {
				Statement st = conexao.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_READ_ONLY);
				String sql = "SELECT * FROM public.usuario" + ((orderBy.trim().length() == 0) ? "" : (" ORDER BY " + orderBy));
				System.out.println(sql);
				ResultSet rs = st.executeQuery(sql);	           
		        while(rs.next()) {	            	
		...
		public List<Pessoa> getSexoMasculino() {
			...
			try {
				Statement st = conexao.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_READ_ONLY);
				String sql = "SELECT * FROM public.usuario WHERE sexo LIKE 'M'";
				System.out.println(sql);
				ResultSet rs = st.executeQuery(sql);	           
		        while(rs.next()) {	            	
		        	Pessoa u = new Pessoa(rs.getInt("codigo"), rs.getString("login"), rs.getString("senha"), rs.getString("sexo").charAt(0));
		...
		public boolean update(Pessoa usuario) {
			...
			try {  
				Statement st = conexao.createStatement();
				String sql = "UPDATE public.usuario SET login = '" + usuario.getLogin() + "', senha = '"  
		...
		public boolean autenticar(String login, String senha) {
			...
			try {
				Statement st = conexao.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_READ_ONLY);
				String sql = "SELECT * FROM public.usuario WHERE login LIKE '" + login + "' AND senha LIKE '" + senha  + "'";
				System.out.println(sql);
				ResultSet rs = st.executeQuery(sql);
				resp = rs.next();
		        st.close();
